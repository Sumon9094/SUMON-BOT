module.exports = function ({ api, models }) {
            ...
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    ...
                    for (const item of checktt.day) {
                        const userName = await Users.getNameUser(item.id) || 'JOY';
                ...
                if (day_now == 1) {
                    ...
                    for (const checkttFile of checkttData) {
                        ...
                        for (const item of checktt.week) {
                            const userName = await Users.getNameUser(item.id) || 'JOY HU YAR';
                            const itemToPush = item;
                            itemToPush.name = userName;
                            storage.push(itemToPush);
                        };
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ JOY ]');
            ...
            logger.loader(global.getText('listen', 'loadedEnvironmentUser')), logger(global.getText('listen', 'successLoadEnvironment'), '[ JOY ]');
        } catch (error) {
            return logger.loader(global.getText('listen', 'failLoadEnvironment', error), 'error');
        }
    }());
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, "[ JOY ]");
        ...
        for (el of temp) {
            ...
            if ("ATTACHMENT" in el) {
                ...
                for (a of el.ATTACHMENT) {
                    ...
                    fs.writeFileSync(__dirname + `/../models/commands/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
                    out.attachment.push(fs.createReadStream(__dirname + `/../models/commands/cache/${a.fileName}`));
                }
            }
            console.log(out);
            if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
            api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../models/commands/cache/${a.fileName}`)) : "");
        }
